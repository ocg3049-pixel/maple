...
| M2 | 자동사냥 시스템 구현 (자동 이동·자동 스킬·방치 루프 — M1 밸런스의 전제) → `MapleIdle-M2-GDD.md` | 🔨 active (Phase 2/3 done) |
| M3 | **밸런스 수렴** — 10직업 이론 DPS를 목표 대역으로 수렴 · 몬스터 HP 공식 확정 · 자동사냥 실측 기반 재캘리브레이션 (`LevelCurve`/`RegionTier` 재보정) | candidate |
...
> 📌 2026-08-10 M3 재정의: 원래 M3는 "직업별 밸런스 확장 — 직업당 1슬롯"이었으나,
>   **10직업 스킬 트리·직업별 자동사냥 로테이션·이론 DPS표·몬스터 HP 커브·전투력 대개편이 08-04~08-10에 이미 구현**됐다
>   (구현 기록은 `Archive/As-built.md`, 수치 원본은 `Docs/MapleIdle-Balance-Worksheet.md`).
>   남은 일은 "만드는 것"이 아니라 "**맞추는 것**"이라 M3를 밸런스 수렴으로 재정의하고,
>   Backlog의 "밸런스 실측 재보정"을 M3 본체로 흡수했다.
